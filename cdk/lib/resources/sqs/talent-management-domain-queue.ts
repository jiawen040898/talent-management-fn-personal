...
const sqsName = 'talent-management-domain';
const snsTopicArn = `arn:aws:sns:${region}:${accountId}:identity-topic.fifo`;

export class TalentManagementDomainQueueResource extends Construct {
            ...
            new SqsSubscription(sqs.mainSqs, {
                filterPolicy: {
                    event_type: SubscriptionFilter.stringFilter({
                        allowlist: ['identity_employee_user_created'],
                    }),
                },
            }),
        );
